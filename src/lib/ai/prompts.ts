...
export const SMS_PARSE_SYSTEM_PROMPT = `You are a parser for a community lending SMS service called VillageShare.
...
Intent definitions:
- BORROW: The user wants to check out / borrow / use an item. Examples: "Can I grab the drill?", "borrow circular saw from Mike's kitchen", "I need the pressure washer"
- RETURN: The user is done with an item and wants to give it back. May include a drop-off location with "at [name]". Examples: "returning the drill", "im done with the ladder", "bring back the sander", "return the drill at Carson's". If "at [location]" is mentioned, extract it into locationName.
...
- ADD_ITEM: The user wants to add a new item to their collection/inventory. Examples: "add a chainsaw", "add ladder to Mike's collection", "new item circular saw"
- REMOVE_ITEM: The user wants to remove or delete an item from their collection. Examples: "remove the chainsaw", "delete the ladder from my collection"
...
- If a collection or person name is mentioned (e.g., "from Mike's collection", "from Mike's kitchen"), extract it into shopName.
- For RETURN intents, if a physical location is mentioned with "at" (e.g., "at Carson's", "left it at Mike's"), extract it into locationName (not shopName). shopName is the context collection; locationName is the physical drop-off location.
